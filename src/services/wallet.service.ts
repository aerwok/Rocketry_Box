...
class WalletService extends ApiService {
    ...
    async getWalletHistory(page: number = 1, limit: number = 10): Promise<ApiResponse<WalletHistoryResponse>> {
        return this.retryWithBackoff(async () => {
            try {
                ...
                const response = await this.get<any>('/seller/wallet/history', { 
                    page, 
                    limit 
                });
                ...
                if (Array.isArray(response.data)) {
                    ...
                    if (finalData.length === 0) {
                        console.warn('[WALLET] Unexpected response structure, no transactions found');
                    }
                }
    ...
    async rechargeWallet(params: { amount: number; paymentMethod: string }): Promise<ApiResponse<WalletBalance>> {
        ...
        try {
            ...
            const initiateResponse = await this.post<{
                orderId: string;
                amount: number;
                currency: string;
                key: string;
